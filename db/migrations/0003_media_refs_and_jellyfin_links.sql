-- ReelHouse migration 0003: stable media identity and Jellyfin account links.
--
...
-- authority's external id as text, so Jellyfin internal row ids are
-- data here, never identity. ReelHouse tables reference media_item_ref.id,
-- which stays stable even if the external id is later remapped.
--
