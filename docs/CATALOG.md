# ReelHouse media catalog — schema, identity, and sync (RH-0031 full, RH-0039 incremental)
...
| Path | Role |
|---|---|
| `db/migrations/0002–0005, 0007_media_catalog_*.sql` | Versioned catalog schema, including the incremental change-history family (see `docs/DATABASE.md` for the migration contract). |
...
| `src/lib/catalog/changes.ts` | Shared change-detection core (RH-0039): content diffing against stored rows, transition planning, duplicate policy, watermark math. |
...
| `src/lib/catalog/incremental.ts` | Incremental refresh (RH-0039): watermark-windowed deltas plus presence sweeps. |
...
| `media_item_changes` (RH-0039) | Append-only change history: one row per genuine state transition (`added`/`updated`/`removed`/`restored`) with the source revision (Etag, else DateLastSaved), the source-provided `observed_at`, and — for updates — a bounded, deterministically ordered `changed_fields` list. |
| `media_sync_state` (RH-0039) | The incremental watermark per source: the newest source `DateLastSaved` a successful run has fully covered. Advances only on success and only forward (`GREATEST`). |
| `media_item_quarantine` (RH-0039) | Conflicting source identities (one Jellyfin id reported with differing placement/content), isolated non-destructively for the repair workbench (RH-0036). At most one open quarantine per identity; re-occurrences bump `occurrences`. |
...
## Incremental refresh (`npm run catalog:sync -- --incremental`, RH-0039)
...
only — plus the RH-0039 suite: migration 0007, the no-baseline fail-closed
