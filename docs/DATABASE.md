# ReelHouse PostgreSQL 18 — connectivity, roles, and migrations (RH-0024)
...
| `src/lib/db/migrator.ts` | Versioned migration loader/planner/runner: checksums, advisory lock, per-migration transactions. |
| `db/migrations/NNNN_name.sql` | Ordered, forward-only migrations. `{{app_role}}` is the only supported placeholder. |
...
`jellyfin` is reported alongside (`unconfigured` / `reachable` with
name+version+latency / `unreachable` with a redacted detail) and never flips
the overall status: an unreachable Jellyfin degrades the library source to
demo mode, it does not make ReelHouse unhealthy.
