...
export const PATCH = householdHandler(async (request: NextRequest, context: RouteContext) => {
  ...
  return runMutation(pool, {
    scope: "watchlists.rename",
    key: readIdempotencyKey(request),
    fingerprint: fingerprintRequest("watchlists.rename", { profileId, watchlistId, name }),
    apply: async (db) => {
...
export const DELETE = householdHandler(async (request: NextRequest, context: RouteContext) => {
  ...
  await requireProfile(pool, profileId);
  return runMutation(pool, {
    scope: "watchlists.delete",
    key: readIdempotencyKey(request),
    fingerprint: fingerprintRequest("watchlists.delete", { profileId, watchlistId }),
    apply: async (db) => {
