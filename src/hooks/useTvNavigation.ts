...
type TvNavigationOptions = {
  ...
  onBack: () => boolean;
  /**
   * Enter pressed inside a text field (the search commit gesture).
   * Return true when handled; Enter on controls stays native.
   */
  onEnterInText?: () => boolean;
};
 ...
 */
export function useTvNavigation({ onBack, onEnterInText }: TvNavigationOptions): void {
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "Enter" && text && onEnterInText?.()) {
        e.preventDefault();
        markKeyboard();
        return;
      }
