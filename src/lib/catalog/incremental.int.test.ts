// Deterministic PostgreSQL + Jellyfin integration evidence for the RH-0039
...
//
// Scenarios: migration 0007 (change history, watermark, quarantine) applies
...
//
// This file uses its own temporary database (reelhouse_rh0039_tmp) so it
// cannot interfere with the db/full-sync/household suites even when
// node --test runs all four files as parallel processes.
...
const TEMP_DB = "reelhouse_rh0039_tmp";
...
test("migration 0007 adds change history, watermark, and quarantine under the same grants model", async (t) => {
