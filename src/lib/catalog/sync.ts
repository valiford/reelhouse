...
//
// Design (docs/CATALOG_SYNC.md, RH-0026):
    ...
    try {
      if (options.mode === "rebuild") {
        ...
        try {
          ...
          // Plain DELETE, not TRUNCATE, so the least-privilege sync role
          // (DML only by design) can run a rebuild; library deletion
          // cascades to items and join rows via the schema FKs.
          await pool.query("DELETE FROM catalog_quarantine");
          await pool.query("DELETE FROM catalog_library");
          await pool.query("DELETE FROM catalog_genre");
          await pool.query("DELETE FROM catalog_studio");
          await pool.query("DELETE FROM catalog_person");
          await pool.query(
...
async function upsertLibrary(pool: Pool, model: CatalogLibraryModel, seenAt: Date): Promise<string> {
  ...
  await pool.query(
    `UPDATE catalog_library
        SET name = $2, collection_type = $3, primary_image_tag = $4, content_hash = $5,
            last_seen_at = $6, missing_since = NULL, retired_at = NULL
      WHERE id = $1`,
