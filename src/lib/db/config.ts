...
//
// Pure parsing/validation logic: no I/O, no imports, so it runs under
...
//
// Secrets are env-only: the URL arrives through DATABASE_URL at process
// start, is never written to disk by this module, and is redacted by
// redactDatabaseUrl()/describeDatabaseConfig() everywhere it could reach a
// log, a response body, or an error message.
//
// Two URLs exist by design:
//   DATABASE_URL          — the application's least-privilege role (DML only).
//   DATABASE_MIGRATE_URL  — optional owner/migrator role override used by the
//                           versioned migration runner; falls back to
//                           DATABASE_URL when unset (local dev convenience).
...
export function loadDatabaseConfig(env: Record<string, string | undefined> = {}): DatabaseConfigResult {
  return loadDatabaseUrl(env, "DATABASE_URL");
}

// Same contract as loadDatabaseConfig, for the migrator role URL.
export function loadMigrateConfig(env: Record<string, string | undefined> = {}): DatabaseConfigResult {
  const migrateUrl = env.DATABASE_MIGRATE_URL?.trim();
  if (migrateUrl) return loadDatabaseUrl({ ...env, DATABASE_URL: migrateUrl }, "DATABASE_MIGRATE_URL");
...
function loadDatabaseUrl(
  ...
  try {
    ...
    return { kind: "invalid", errors: [`${varName} is not a valid URL`] };
  }

  if (url.protocol !== "postgres:" && url.protocol !== "postgresql:") {
    return { kind: "invalid", errors: [`${varName} must use postgres:// or postgresql:// (got ${url.protocol})`] };
  }

  const errors: string[] = [];
  if (!url.hostname) errors.push(`${varName} has no host`);

  const database = url.pathname.replace(/^\/+/, "");
  if (!database) errors.push(`${varName} has no database name`);

  const port = url.port ? Number(url.port) : DATABASE_DEFAULTS.port;
  if (!Number.isInteger(port) || port < 1 || port > 65535) errors.push(`${varName} has an invalid port`);
