...
// - Idempotent: a full re-run against an up-to-date database applies nothing
...
//   concurrently.
// - Provenance: migrations never rewrite or re-derive existing rows; they
//   only create/alter ReelHouse-owned objects and record their own history.
...
// run. Never contacts the database. Reused by the readiness summary so
// /api/health and the runner can never disagree about what "pending" means.
...
// Read-only status through the owner/migrator role (used by the CLI/tests).
// Takes the same advisory lock as a run so it cannot observe a half-applied
// state; it never applies anything.
...
// Read-only migration summary for /api/health. Unlike checkMigrations this
// reuses the caller's query function (the application pool), so readiness
// never opens a second connection and never takes the advisory lock; the
// application role only needs the read-only bookkeeping grant from 0001.
// Every failure degrades to `unknown` with a bounded detail — the summary is
// diagnostic and must never throw out of a health request.
export async function summarizeMigrations(
  migrationsDir: string,
  queryApplied: () => Promise<AppliedMigration[]>
  ...
  try {
    applied = await queryApplied();
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    // 42P01 = undefined_table: the bookkeeping table does not exist yet, so
    // nothing has been applied on this database — that is a normal state.
  ...
  if (applied.length) summary.lastVersion = applied[applied.length - 1].version;
  return summary;
}
