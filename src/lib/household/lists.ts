// Household lists persistence (RH-0027): favorites, watchlists, and curated
// collections + membership over the household schema in the `reelhouse`
// database.
//
// This module sits beside store.ts (profiles / preferences / watch-state /
// links) and shares its primitives: the injected SqlRunner, transact() for
// atomic multi-step operations, and resolveMediaRef() for the stable media
// bridge.
//
// Semantics:
...
// - Item positions need not be contiguous: an explicit position splices
//   (neighbors shift), reads order by (position, added_at, media_ref_id),
//   and reorder renumbers 1..n only when the submitted set equals the
...
import type { ValidatedMediaRef } from "./model.ts";
...
// create (double-tap, replay) return the existing list.
...
// Cascades collection items. Deleting a profile never cascades here:
// created_by_profile_id is ON DELETE SET NULL provenance (RH-0027 migration
// 0006), so the household's collections survive their creator.
