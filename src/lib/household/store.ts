// ReelHouse-owned household state persistence (RH-0027).
//
...
// bridge, and replay-detection keys — all in the PG18 `reelhouse` database,
// all reached only through this server-side layer (clients never see SQL or
// credentials, per docs/ARCHITECTURE.md).
//
// Split design: the runner is injected, this module is deliberately free of
// `server-only` and the `@/` alias, and its pg import is type-only — so
// route handlers, the Next.js server runtime, and plain-Node tests (unit
// fixtures or a real disposable PG18) can all drive the same functions.
// Transactional callers pass a transaction-scoped runner via transact();
// single-statement callers pass the pool or a client.
    ...
    const result = await tx.query<WatchStateRow>(
      `INSERT INTO watch_state (profile_id, media_ref_id, position_ticks, duration_ticks, completed, last_played_at)
       ...
       RETURNING profile_id,
                 ...
                 position_ticks, duration_ticks, completed, last_played_at, updated_at`,
      [
        input.profileId,
        mediaRefId,
        input.positionTicks,
...
// Canonical fingerprint over the semantic input (key order is fixed by the
// literal construction below), so a replayed request with reformatted
// whitespace still counts as the same write.
